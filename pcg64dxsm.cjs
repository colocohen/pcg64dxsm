...
class PCG64DXSM {
  ...
  intBelow(bound) {
    ...
    if (typeof bound === 'bigint') {
      ...
      if (typeof bound !== 'number' || bound <= 0 || !Number.isFinite(bound)) {
        throw new Error('bound must be positive number or BigInt');
      }
  ...
  destroy() {
    ...
    if (this._ptr) {
      _cleanup.unregister(this);
      W.free_state(this._ptr);
      this._ptr = 0;
    }
    if (this._scratchPtr) {
      _cleanup.unregister(this._scratchToken);
      W.free_state(this._scratchPtr);
      this._scratchPtr = 0;
    }
  }
  ...
  uuid4() {
    ...
    b[6] = (b[6] & 0x0f) | 0x40;
    b[8] = (b[8] & 0x3f) | 0x80;
    const hex = (u) => u.toString(16).padStart(2, '0');
    return (
